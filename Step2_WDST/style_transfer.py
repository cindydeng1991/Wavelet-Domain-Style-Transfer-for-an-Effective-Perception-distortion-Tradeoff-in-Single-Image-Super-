...
class StyleTransfer:
    ...
    def _build_graph(self):
        ...
        self.L1norm = tf.reduce_sum(tf.abs((self.x)))
        self.L_total = alpha*L_content + beta*L_style +gamma*tf.reduce_sum(tf.abs((self.x)))
    ...
    def update(self):
        ...
        global _iter
        _iter = 0
        def callback(tl, cl, sl, l1):
            global _iter
            print('iter : %4d, ' % _iter, 'L_total : %g, L_content : %g, L_style : %g, L1_norm : %g' % (tl, cl, sl, l1))
            _iter += 1
        ...
        optimizer.minimize(self.sess,feed_dict={self.a:self.a0, self.p:self.p0},
                           fetches=[self.L_total, self.L_content, self.L_style, self.L1norm], loss_callback=callback)
