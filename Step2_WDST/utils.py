...
def load_image(filename, shape=None, max_size=None):
    ...
    if max_size is not None:
        ...
        # Resize the image.
        image = image.resize(size, PIL.Image.BICUBIC) # PIL.Image.LANCZOS is one of resampling filter

    if shape is not None:
        image = image.resize(shape, PIL.Image.BICUBIC) # PIL.Image.LANCZOS is one of resampling filter
